 ...
 */
@Component
public class NodeRPCConnectionNodeNotary extends NodeRPCConnectionNode implements AutoCloseable {

    // The host of the node we are connecting to.
    @Value("${notary.rpc.host}")
    private String host;
    // The RPC port of the node we are connecting to.
    @Value("${notary.rpc.auth.username}")
    private String username;
    // The username for logging into the RPC client.
    @Value("${notary.rpc.auth.password}")
    private String password;
    // The password for logging into the RPC client.
    @Value("${notary.rpc.port}")
    private int rpcPort;

    private static final Logger logger = LoggerFactory.getLogger(NodeRPCConnectionNodeNotary.class);

    private static final NodeNameEnum NODE_NAME = NodeNameEnum.NOTARY;
